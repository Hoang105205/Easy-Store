...
public class DbConfigManager
{
    ...
    public static async Task ShowConfigDialogAsync(XamlRoot xamlRoot)
    {
        ...
        string savedDbUrl = settings.Values["DbConnectionString"]?.ToString() ?? "";
        string savedUri = settings.Values["SupabaseUri"]?.ToString() ?? "";
        string savedApiKey = settings.Values["SupabaseApiKey"]?.ToString() ?? "";
        ...
        if (result == ContentDialogResult.Primary)
        {
            ...
            {
                settings.Values["DbConnectionString"] = newDbUrl;
                settings.Values["SupabaseUri"] = newUri;
                settings.Values["SupabaseApiKey"] = newApiKey;
