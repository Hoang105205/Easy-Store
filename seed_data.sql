-- Lệnh này sẽ quét sạch dữ liệu của các bảng, đồng thời reset lại các cột ID tự tăng (nếu có)
-- CASCADE giúp tự động xóa dữ liệu ở các bảng con (như OrderItems, ImportLogDetails, ProductImages)
TRUNCATE TABLE "Users", "Categories", "Products", "ProductImages", "ImportLogs", "ImportLogDetails", "Orders", "OrderItems" RESTART IDENTITY CASCADE;
...
-- Đã thêm 2 cột: AvailableStockQuantity và MinimumStockQuantity
...
-- 6. BƠM TÀI KHOẢN USER (Tuyệt đối không đổi)
